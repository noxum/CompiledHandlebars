...
namespace CompiledHandlebars.Compiler.CodeGeneration
{
    public class CompilationState
    {
        ...
        internal Stack<Context> EqualsStack { get; set; } = new Stack<Context>();
        ...
        internal void PromiseEqualsCheck(Context contextToCheck, IfType ifType = IfType.If)
        {
            contextToCheck.Truthy = ifType == IfType.If;
            EqualsStack.Push(contextToCheck);
        }

        internal void DoEqualsCheck(List<StatementSyntax> equalsBlock, List<StatementSyntax> elseBlock = null)
        {
            var contextToCheck = EqualsStack.Pop();
            IfStatementSyntax ifStatement;
            if (EqualsStack.Any())
                ifStatement = SyntaxHelper.IfEquals(GetQueryElements(EqualsStack.Peek(), contextToCheck));
            else
                ifStatement = SyntaxHelper.IfEquals(GetQueryElements(null, contextToCheck));
            if (ifStatement == null)
            {
                if (elseBlock != null)
                    AddTypeError("Unreachable 'else' Block", HandlebarsTypeErrorKind.UnreachableCode);
                resultStack.Peek().AddRange(equalsBlock);
            }
            else
            {
                if (elseBlock != null)
                {
                    resultStack.Peek().Add(
                      ifStatement.WithStatement(SyntaxFactory.Block(equalsBlock)).
